...
from trading_bot.main import _eligible_timeframes, _fetch_key, _fetch_plan, _next_scan_delay_seconds
...
class ScanPlannerTests(unittest.TestCase):
    ...
    def test_reserves_capacity_for_dynamic_pairs_when_core_pairs_are_due(self) -> None:
        instruments = [
            Instrument(symbol="CORE_FIRST", label="CORE_FIRST", asset_name="Core first"),
            Instrument(symbol="CORE_SECOND", label="CORE_SECOND", asset_name="Core second"),
            Instrument(symbol="DYNAMIC", label="DYNAMIC", asset_name="Dynamic"),
        ]
        state = SimpleNamespace(last_fetched_at=lambda _: None, last_failed_at=lambda _: None)
        config = SimpleNamespace(
            app=SimpleNamespace(
                historical_request_budget=2,
                dynamic_request_reserve=1,
                timeframe_refresh_seconds={"1h": 3600},
            ),
            universe=SimpleNamespace(enabled=True, core_symbols=["CORE_FIRST", "CORE_SECOND"]),
        )

        plan = _fetch_plan(config, state, instruments, ["1h"], datetime(2026, 7, 17, 10, tzinfo=timezone.utc))

        self.assertEqual(
            [(instrument.symbol, timeframe) for instrument, timeframe in plan],
            [("CORE_FIRST", "1h"), ("DYNAMIC", "1h")],
        )
    ...
    def test_limits_timeframes_to_the_patterns_that_can_evaluate_each_symbol(self) -> None:
        intraday = Instrument(symbol="INTRADAY", label="INTRADAY", asset_name="Intraday")
        structural = Instrument(symbol="STRUCTURAL", label="STRUCTURAL", asset_name="Structural")
        config = SimpleNamespace(
            symbol_lists={"intraday_focus": ["INTRADAY"]},
            patterns={
                "reversion": SimpleNamespace(enabled=True, settings={"symbol_list": "intraday_focus", "timeframes": ["1h"]}),
                "breakout": SimpleNamespace(enabled=True, settings={"timeframes": ["4h", "1d"]}),
            },
        )

        eligible = _eligible_timeframes(config, [intraday, structural])

        self.assertEqual(eligible, {"INTRADAY": ["1h", "4h", "1d"], "STRUCTURAL": ["4h", "1d"]})
