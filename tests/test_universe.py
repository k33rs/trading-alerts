...
from trading_bot.universe import UniverseStore, _core_instruments, _multi_timeframe_trend_status, _trend_status, _watchlist_candidates, active_universe, refresh_universe, universe_instrument_payload
...
class UniverseStoreTests(unittest.TestCase):
    ...
    def test_enabled_pattern_symbol_list_members_bypass_trend_qualification(self) -> None:
        core = Instrument(symbol="CORE", label="CORE", asset_name="Core")
        intraday = Instrument(symbol="INTRADAY", label="INTRADAY", asset_name="Intraday")
        config = SimpleNamespace(
            watchlist=[core, intraday],
            universe=SimpleNamespace(enabled=True, core_symbols=["CORE"]),
            symbol_lists={"intraday_focus": ["INTRADAY"]},
            patterns={
                "reversion": SimpleNamespace(
                    enabled=True,
                    settings={"symbol_list": "intraday_focus"},
                )
            },
        )

        instruments = _core_instruments(config)

        self.assertEqual([instrument.symbol for instrument in instruments], ["CORE", "INTRADAY"])

    def test_active_universe_adds_pattern_required_symbols_to_fresh_snapshot(self) -> None:
        core = Instrument(symbol="CORE", label="CORE", asset_name="Core")
        intraday = Instrument(symbol="INTRADAY", label="INTRADAY", asset_name="Intraday")
        with TemporaryDirectory() as directory:
            database_path = Path(directory) / "universe.db"
            UniverseStore(database_path).replace([core])
            config = SimpleNamespace(
                watchlist=[core, intraday],
                symbol_lists={"intraday_focus": ["INTRADAY"]},
                patterns={
