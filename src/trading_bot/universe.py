...
def active_universe(config: BotConfig) -> list[Instrument]:
    ...
    static_instruments = _core_instruments(config)
    if not snapshot:
        return static_instruments
    snapshot_keys = {_instrument_key(instrument) for instrument in snapshot}
    missing_static_instruments = [
        instrument for instrument in static_instruments if _instrument_key(instrument) not in snapshot_keys
    ]
    return _order_instruments([*snapshot, *missing_static_instruments])
...
def _core_instruments(config: BotConfig) -> list[Instrument]:
    ...
    required_symbols = [*config.universe.core_symbols, *pattern_required_symbols(config)]
    return [catalog[symbol] for symbol in dict.fromkeys(required_symbols)]


def pattern_required_symbols(config: BotConfig) -> list[str]:
    required_symbols: list[str] = []
    patterns = getattr(config, "patterns", {})
    symbol_lists = getattr(config, "symbol_lists", {})
    for pattern in patterns.values():
        if not pattern.enabled:
            continue
        symbols = pattern.settings.get("only_symbols", [])
        if not symbols:
            symbol_list_name = pattern.settings.get("symbol_list")
            symbols = symbol_lists.get(symbol_list_name, []) if symbol_list_name else []
        required_symbols.extend(symbols)
    return list(dict.fromkeys(required_symbols))
