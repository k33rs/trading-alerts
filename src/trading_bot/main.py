...
from trading_bot.universe import active_universe, pattern_required_symbols
...
def _eligible_timeframes(config: BotConfig, instruments: list) -> dict[str, list[str]]:
    eligible: dict[str, list[str]] = {instrument.symbol: [] for instrument in instruments}
    for pattern in config.patterns.values():
        if not pattern.enabled:
            continue
        only_symbols = pattern.settings.get("only_symbols", [])
        symbol_list_name = pattern.settings.get("symbol_list")
        allowed_symbols = set(only_symbols or config.symbol_lists.get(symbol_list_name, []))
        for instrument in instruments:
            if allowed_symbols and instrument.symbol not in allowed_symbols:
                continue
            for timeframe in pattern.settings.get("timeframes", []):
                if timeframe not in eligible[instrument.symbol]:
                    eligible[instrument.symbol].append(timeframe)
    return eligible
...
def _fetch_plan(
    ...
    state: AlertState,
    instruments: list,
    timeframes: list[str] | dict[str, list[str]],
    now: datetime,
) -> list[tuple[object, str]]:
    app = config.app
    budget = max(1, int(getattr(app, "historical_request_budget", 20)))
    dynamic_request_reserve = min(
        budget,
        max(0, int(getattr(app, "dynamic_request_reserve", 0))),
    )
    ...
    core_symbols = (
        set(getattr(config.universe, "core_symbols", [])) | set(pattern_required_symbols(config))
    ...
    for instrument_index, instrument in enumerate(instruments):
        instrument_timeframes = timeframes.get(instrument.symbol, []) if isinstance(timeframes, dict) else timeframes
        for timeframe_index, timeframe in enumerate(instrument_timeframes):
    ...
    reserved_dynamic_pairs = [pair for pair in due_pairs if pair[0]][:dynamic_request_reserve]
    selected_pair_ids = {id(pair) for pair in reserved_dynamic_pairs}
    remaining_slots = budget - len(reserved_dynamic_pairs)
    selected_pairs = [
        *reserved_dynamic_pairs,
        *[pair for pair in due_pairs if id(pair) not in selected_pair_ids][:remaining_slots],
    ]
    selected_pairs.sort(key=lambda pair: pair[:7])
    return [(instrument, timeframe) for *_, instrument, timeframe in selected_pairs]
...
def _next_scan_delay_seconds(
    ...
    for instrument in instruments:
        instrument_timeframes = timeframes.get(instrument.symbol, []) if isinstance(timeframes, dict) else timeframes
        for timeframe in instrument_timeframes:
            key = _fetch_key(instrument.symbol, timeframe)
            last_fetched_at = state.last_fetched_at(key)
            last_failed_at = state.last_failed_at(key)
            cadence = max(1, int(configured_cadences.get(timeframe, DEFAULT_TIMEFRAME_REFRESH_SECONDS.get(timeframe, 3_600))))
...
def run_scan(
    ...
    eligible_timeframes = _eligible_timeframes(config, instruments)
    fetch_plan = _fetch_plan(config, state, instruments, eligible_timeframes, datetime.now().astimezone())
    total_fetches = sum(len(timeframes) for timeframes in eligible_timeframes.values())
    ...
    try:
        while True:
            ...
            delay_seconds = _next_scan_delay_seconds(
                ...
                _eligible_timeframes(config, active_universe(config)),
                now,
            )
